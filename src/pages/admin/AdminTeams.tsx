...
const AdminTeams: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name || !form.competitionId) return;
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => { resetForm(); setShowForm(true); }} className="flex items-center gap-1 px-4 py-2 bg-[#1E6FF2] text-white rounded-lg text-sm font-medium hover:bg-[#1558CC] transition-colors">
          <Plus className="w-4 h-4" /> Shto Skuadër
        </button>
      </div>
      ...
      <div className="flex gap-2 mb-4">
        <select value={filterComp} onChange={e => setFilterComp(e.target.value)} className="px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white">
          <option value="all">Të gjitha kompeticionet</option>
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-gray-200 p-4 mb-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold text-gray-800">{editId ? 'Edito Skuadrën' : 'Shto Skuadër të Re'}</h3>
              ...
              return (
                <div key={t.id} className="flex items-center justify-between px-4 py-3 hover:bg-gray-50">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => handleEdit(t)} className="p-1.5 text-gray-400 hover:text-[#1E6FF2] rounded"><Pencil className="w-4 h-4" /></button>
                    <button onClick={() => { if (confirm('Fshi skuadrën?')) deleteTeam(t.id); }} className="p-1.5 text-gray-400 hover:text-red-500 rounded"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
              );
            })}
