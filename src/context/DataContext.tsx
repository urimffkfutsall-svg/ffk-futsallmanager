...
import { Season, Competition, Team, Player, Match, Goal, Scorer, PlayerOfWeek, User, StandingsRow, AppSettings, CommissionDecision, WeeklyFormation, TeamOfficial, PlayerDiscipline } from '@/types';
import { v4 as uuidv4 } from 'uuid';
import {
  fetchAllData,
  dbSeasons, dbCompetitions, dbTeams, dbPlayers, dbMatches, dbGoals, dbScorers, dbPlayerOfWeek, dbUsers, dbSettings,
...
interface DataState {
  ...
  weeklyFormations: WeeklyFormation[];
...
interface DataContextType extends DataState {
  ...
  commissionDecisions: CommissionDecision[];
  addCommissionDecision: (d: Omit<CommissionDecision, 'id'>) => void;
  updateCommissionDecision: (d: CommissionDecision) => void;
  deleteCommissionDecision: (id: string) => void;
  weeklyFormations: WeeklyFormation[];
  teamOfficials: TeamOfficial[];
  playerDiscipline: PlayerDiscipline[];
  addWeeklyFormation: (f: Omit<WeeklyFormation, 'id'>) => void;
  updateWeeklyFormation: (f: WeeklyFormation) => void;
  deleteWeeklyFormation: (id: string) => void;
  teamOfficials: TeamOfficial[];
  addTeamOfficial: (o: Omit<TeamOfficial, 'id'>) => void;
  updateTeamOfficial: (o: TeamOfficial) => void;
  deleteTeamOfficial: (id: string) => void;
  playerDiscipline: PlayerDiscipline[];
  addPlayerDiscipline: (d: Omit<PlayerDiscipline, 'id'>) => void;
  deletePlayerDiscipline: (id: string) => void;
  // Standings
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setState({
        ...
        users: data.users.length > 0 ? data.users : [{ id: uuidv4(), username: 'datapos', password: 'datapos1', role: 'admin' }],
        settings: data.settings,
        commissionDecisions: JSON.parse(localStorage.getItem('ffk_cd') || '[]'),
          weeklyFormations: JSON.parse(localStorage.getItem('ffk_wf') || '[]'),
          teamOfficials: JSON.parse(localStorage.getItem('ffk_to') || '[]'),
          playerDiscipline: JSON.parse(localStorage.getItem('ffk_pd') || '[]'),
      });
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const tables = ['seasons', 'competitions', 'teams', 'players', 'scorers', 'player_of_week', 'users'];
    const stateKeys: Record<string, keyof DataState> = {
      seasons: 'seasons', competitions: 'competitions', teams: 'teams', players: 'players',
      scorers: 'scorers', player_of_week: 'playersOfWeek', users: 'users',
    };
  ...
  const getAggregatedScorers = (compId?: string) => {
    ...
    state.goals.forEach(g => {
      ...
      if (!match) return;
      if (compId && match.competitionId !== compId) return;
  ...
  const addWeeklyFormation = (f: Omit<WeeklyFormation, 'id'>) => {
    const newItem: WeeklyFormation = { ...f, id: crypto.randomUUID() };
    update(prev => ({ ...prev, weeklyFormations: [...(prev.weeklyFormations || []), newItem] }));
    try { const s = JSON.parse(localStorage.getItem('ffk_wf') || '[]'); localStorage.setItem('ffk_wf', JSON.stringify([...s, newItem])); } catch {}
  };
  const updateWeeklyFormation = (f: WeeklyFormation) => {
    update(prev => ({ ...prev, weeklyFormations: (prev.weeklyFormations || []).map(x => x.id === f.id ? f : x) }));
    try { const s = JSON.parse(localStorage.getItem('ffk_wf') || '[]'); localStorage.setItem('ffk_wf', JSON.stringify(s.map((x: WeeklyFormation) => x.id === f.id ? f : x))); } catch {}
  };
  const deleteWeeklyFormation = (id: string) => {
    update(prev => ({ ...prev, weeklyFormations: (prev.weeklyFormations || []).filter(x => x.id !== id) }));
    try { const s = JSON.parse(localStorage.getItem('ffk_wf') || '[]'); localStorage.setItem('ffk_wf', JSON.stringify(s.filter((x: WeeklyFormation) => x.id !== id))); } catch {}
  };
  const addCommissionDecision = (d: Omit<CommissionDecision, 'id'>) => {
    const newItem: CommissionDecision = { ...d, id: uuidv4() };
    update(prev => ({ ...prev, commissionDecisions: [...prev.commissionDecisions, newItem] }));
    try { const s = JSON.parse(localStorage.getItem('ffk_cd') || '[]'); localStorage.setItem('ffk_cd', JSON.stringify([...s, newItem])); } catch {}
  };
  const updateCommissionDecision = (d: CommissionDecision) => {
    update(prev => ({ ...prev, commissionDecisions: prev.commissionDecisions.map(x => x.id === d.id ? d : x) }));
    try { const s = JSON.parse(localStorage.getItem('ffk_cd') || '[]'); localStorage.setItem('ffk_cd', JSON.stringify(s.map((x: CommissionDecision) => x.id === d.id ? d : x))); } catch {}
  };
  const deleteCommissionDecision = (id: string) => {
    update(prev => ({ ...prev, commissionDecisions: prev.commissionDecisions.filter(x => x.id !== id) }));
    try { const s = JSON.parse(localStorage.getItem('ffk_cd') || '[]'); localStorage.setItem('ffk_cd', JSON.stringify(s.filter((x: CommissionDecision) => x.id !== id))); } catch {}
  };
  const addTeamOfficial = (o: Omit<TeamOfficial, 'id'>) => {
    const item: TeamOfficial = { ...o, id: crypto.randomUUID() };
    update(prev => ({ ...prev, teamOfficials: [...(prev.teamOfficials || []), item] }));
    try { const s = JSON.parse(localStorage.getItem('ffk_to') || '[]'); localStorage.setItem('ffk_to', JSON.stringify([...s, item])); } catch {}
  };
  const updateTeamOfficial = (o: TeamOfficial) => {
    update(prev => ({ ...prev, teamOfficials: (prev.teamOfficials || []).map(x => x.id === o.id ? o : x) }));
    try { const s = JSON.parse(localStorage.getItem('ffk_to') || '[]'); localStorage.setItem('ffk_to', JSON.stringify(s.map((x: TeamOfficial) => x.id === o.id ? o : x))); } catch {}
  };
  const deleteTeamOfficial = (id: string) => {
    update(prev => ({ ...prev, teamOfficials: (prev.teamOfficials || []).filter(x => x.id !== id) }));
    try { const s = JSON.parse(localStorage.getItem('ffk_to') || '[]'); localStorage.setItem('ffk_to', JSON.stringify(s.filter((x: TeamOfficial) => x.id !== id))); } catch {}
  };
  const addPlayerDiscipline = (d: Omit<PlayerDiscipline, 'id'>) => {
    const item: PlayerDiscipline = { ...d, id: crypto.randomUUID() };
    update(prev => ({ ...prev, playerDiscipline: [...(prev.playerDiscipline || []), item] }));
    try { const s = JSON.parse(localStorage.getItem('ffk_pd') || '[]'); localStorage.setItem('ffk_pd', JSON.stringify([...s, item])); } catch {}
  };
  const deletePlayerDiscipline = (id: string) => {
    update(prev => ({ ...prev, playerDiscipline: (prev.playerDiscipline || []).filter(x => x.id !== id) }));
    try { const s = JSON.parse(localStorage.getItem('ffk_pd') || '[]'); localStorage.setItem('ffk_pd', JSON.stringify(s.filter((x: PlayerDiscipline) => x.id !== id))); } catch {}
  };
  ...
  const value: DataContextType = {
    ...
    addCommissionDecision, updateCommissionDecision, deleteCommissionDecision,
    weeklyFormations: state.weeklyFormations,
    addWeeklyFormation, updateWeeklyFormation, deleteWeeklyFormation,
      teamOfficials: state.teamOfficials || [], addTeamOfficial, updateTeamOfficial, deleteTeamOfficial,
      playerDiscipline: state.playerDiscipline || [], addPlayerDiscipline, deletePlayerDiscipline,
    calculateStandings,
    getHeadToHead,
    refreshData
  };
